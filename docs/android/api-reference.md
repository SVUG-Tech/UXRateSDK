...
|--------|-----------|-------------|
| `configure` | `configure(application: Application, apiKey: String, autoTrackScreens: Boolean = true, theme: SDKTheme = AUTO, currentActivity: Activity? = null)` | Initializes the SDK. Call once in `Application.onCreate()`. The backend is auto-resolved from the `apiKey` prefix (`uxr_…` → production, `uxr_dev_…` → dev, `uxr_loc_…` → local). Pass `currentActivity` when calling from single-Activity hosts (React Native, Flutter) so the overlay attaches immediately. |
