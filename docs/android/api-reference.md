...
|--------|-----------|-------------|
| `configure` | `configure(application: Application, apiKey: String, autoTrackScreens: Boolean = true, mockScreens: List<String>? = null, overlapStrategy: OverlapStrategy = SHOW_FIRST, theme: SDKTheme = AUTO, currentActivity: Activity? = null)` | Initializes the SDK. Call once in `Application.onCreate()`. The backend is auto-resolved from the `apiKey` prefix (`uxr_…` → production, `uxr_dev_…` → dev, `uxr_loc_…` → local). Pass `apiKey = "mock"` to use the in-memory mock service. `mockScreens` specifies which screens trigger mock surveys. Pass `currentActivity` when calling from single-Activity hosts (React Native, Flutter) so the overlay attaches immediately. |
