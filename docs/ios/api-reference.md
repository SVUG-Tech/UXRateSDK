...
|--------|-----------|-------------|
| `configure` | `configure(apiKey:autoTrackScreens:theme:)` | Initializes the SDK. Call once at app launch. The backend is auto-resolved from the `apiKey` prefix (`uxr_…` → production, `uxr_dev_…` → dev, `uxr_loc_…` → local). `autoTrackScreens` (default `true`) swizzles UIViewController for automatic screen detection. `theme` (default `.auto`) sets the color scheme for all SDK UI. |
